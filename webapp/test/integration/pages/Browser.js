		...
		onTheBrowser: {
			...
			actions: {
				...
				iChangeTheHashToTheRememberedItem: function() {
					return this.waitFor({
						success: function() {
							var sObjectId = this.getContext().currentItem.getBindingContext().getProperty("CbArea");
							Opa5.getHashChanger().setHash("/z_cycle_listSet/" + sObjectId);
						}
					});
				},
				...
				iRestartTheAppWithTheRememberedItem: function(oOptions) {
					...
					this.waitFor({
						success: function() {
							sObjectId = this.getContext().currentItem.getBindingContext().getProperty("CbArea");
					...
					return this.waitFor({
						success: function() {
							oOptions.hash = "/z_cycle_listSet/" + encodeURIComponent(sObjectId);
